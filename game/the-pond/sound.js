...
function toggleMute(){if(!muted){popSound.volume=0
bgSound.volume=0
muted=true
localStorage.muted='true'
drawSoundControl()}else{popSound.volume=0.4
bgSound.volume=0.6
muted=false
localStorage.muted='false'
drawSoundControl()}}
function playPop(){popSound.play()}
function drawSoundControl(){if(typeof ctx==='undefined')return
ctx.fillStyle='#111'
...
if(GAME.state==='menu'&&about){ctx.fillStyle='#111'
ctx.fillRect(10,10,50,20)
ctx.fillStyle='#333'
ctx.font='normal 14px sans'
ctx.fillText('By: Zolmeister',10,20)
ctx.font='normal 12px sans'
ctx.fillText('Music: Chrissi J',10,42)}else if(GAME.state==='menu'){ctx.fillStyle='#111'
ctx.fillRect(10,10,50,20)
ctx.fillStyle='#333'
ctx.font='normal 12px sans'
ctx.fillText('About',10,20)}}
function showAbout(){about=true
drawSoundControl()}