function drawMenu(){sizeMenu()
ctx.fillStyle='#111'
ctx.fillRect(0,0,$canv.width,$canv.height)
drawMenuLogo()
drawMenuButton()
drawSoundControl()}
function drawMenuLogo(){var title=GAME.MENU.title
ctx.drawImage(ASSETS[usingSmallLogo?'logoSmall':'logo'],title.x,title.y,title.width,title.height)}
function fadeInMenu(){GAME.state='menu'
GAME.MENU.opacity=0
requestAnimFrame(menuFade)}
function menuFade(){GAME.MENU.opacity+=0.02
drawMenu()
var alpha=1-GAME.MENU.opacity
ctx.fillStyle='rgba(17,17,17,'+(alpha>0?alpha:0)+')'
ctx.fillRect(0,0,$canv.width,$canv.height)
if(GAME.MENU.opacity<1&&GAME.state==='menu'){requestAnimFrame(menuFade)}}
function drawMenuButton(hitting){var button=GAME.MENU.button
ctx.lineWidth=4
ctx.strokeStyle='#444'
...
function sizeMenu(){var title={width:ASSETS[usingSmallLogo?'logoSmall':'logo'].width,height:ASSETS[usingSmallLogo?'logoSmall':'logo'].height,minPaddingX:50,minPaddingY:15,x:null,y:null}
var button={x:null,y:$canv.height/1.8,width:$canv.width*.5,height:$canv.height/6}
...
if(title.width<=300&&!usingSmallLogo){usingSmallLogo=true
sizeMenu()}else if(scale===1&&usingSmallLogo){usingSmallLogo=false
sizeMenu()}}